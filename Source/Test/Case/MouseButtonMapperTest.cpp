...
namespace XidiTest
{
    ...
    {
        constexpr EMouseButton kTestButtons[] = {EMouseButton::Left, EMouseButton::Middle, EMouseButton::Right, EMouseButton::X1, EMouseButton::X2};
    ...
    // Creates and then clones one mouse button mapper for various possible mouse buttons and verifies two things.
    // First, verifies that it does not map to any virtual controller element.
    // Second, verifies that it correctly identifies its target mouse button.
    ...
    // Verifies the nominal behavior in which a mouse button mapper is asked to contribute some arbitrary analog value to a mouse button.
    // Expected behavior is the mouse button is pressed at the extreme analog values and not pressed towards neutral, but the exact transition thresholds are not defined.
    ...
    TEST_CASE(MouseButtonMapper_ContributeFromAnalogValue_Nominal)
    {
        ...
        const MockMouse* const kExpectedMouseSequence[] =  {&expectedMouseStatePressed, &expectedMouseStateUnpressed, &expectedMouseStatePressed, &expectedMouseStatePressed};
        ...
        for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
        {
            ...
            const MockMouse* possibleExpectedStates[2] = {kExpectedMouseSequence[currentSequenceIndex], kExpectedMouseSequence[currentSequenceIndex + 1]};
            ...
            {
                TEST_FAILED_BECAUSE(L"Out-of-sequence mouse state produced by a mouse button mapper with analog input %d.", analogValue);
            }
        }
    ...
    // Verifies a sequence of contributions of extreme and neutral analog values lead to a mouse button mapper pressing, then unpressing, a mouse button.
    ...
    // Verifies the nominal behavior in which a mouse button mapper is asked to contribute some arbitrary button press state to a mouse button.
    TEST_CASE(MouseButtonMapper_ContributeFromButtonValue_Nominal)
    {
        ...
        for (bool buttonIsPressed : kButtonStates)
        {
            ...
            if (true == buttonIsPressed)
                expectedState.SubmitMouseButtonPressedState(kTestMouseButton);
    ...
    // Verifies a sequence of contributions of pressed and unpressed button values lead to a mouse button mapper pressing, then unpressing, a mouse button.
    ...
    // Verifies the nominal behavior in which a mouse button mapper is asked to contribute a trigger value to a mouse button.
    // Expected behavior is the mouse button is not pressed at the start and becomes pressed once the trigger value hits a threshold, but the exact transition point is not defined.
    ...
    TEST_CASE(MouseButtonMapper_ContributeFromTriggerValue_Nominal)
    {
        ...
        for (int32_t triggerValue = kTriggerValueMin; triggerValue <= kTriggerValueMax; ++triggerValue)
        {
            ...
            {
                TEST_FAILED_BECAUSE(L"Out-of-sequence mouse state produced by a mouse button mapper with trigger input %d.", triggerValue);
            }
        }
    ...
    // Verifies a sequence of contributions of extreme and neutral trigger values lead to a mouse button mapper pressing, then unpressing, a mouse button.
    ...
    // Verifies that a mouse button mapper causes a mouse button to be released when it is asked for a neutral contribution.
