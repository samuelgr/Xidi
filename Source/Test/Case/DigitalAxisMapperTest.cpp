 ...
 *   axis but without any analog functionality (i.e. extreme values only).
 *****************************************************************************/
...
namespace XidiTest
{
    ...
    static constexpr TControllerIdentifier kControllerIdentifier = 2;


    // -------- TEST CASES ------------------------------------------------- //

    // Creates one digital axis mapper for each possible virtual button and verifies that each correctly identifies its target virtual controller element.
    ...
    // Verifies the nominal behavior in which a digital axis mapper is asked to contribute some arbitrary analog value to an axis.
    // Sweeps the entire range of possible analog values.
    // The only valid values that can be produced are extreme negative, neutral, and extreme positive, and these values must exist in that order.
    TEST_CASE(DigitalAxisMapper_ContributeFromAnalogValue_Nominal_EntireAxis)
    {
        constexpr EAxis kTargetAxis = EAxis::RotX;
        
        // Specify the allowed values in the order in which they should appear, but without specifying the analog thresholds at which transitions take place.
        // If the current allowed index value is not seen but the next allowed index is, then a transition occurred and we move onto the next allowed index.
        ...
        constexpr int32_t kAllowedValues[] = {kAnalogValueMin, kAnalogValueNeutral, kAnalogValueMax, kAnalogValueMax};
    ...
    // Here, the negative part of the axis is ignored, and the positive part is expected to produce either neutral or extreme positive.
    ...
    // Here, the positive part of the axis is ignored, and the negative part is expected to produce either extreme negative or neutral.
    TEST_CASE(DigitalAxisMapper_ContributeFromAnalogValue_Nominal_HalfAxisNegative)
    {
        ...
        // The positive part of the axis should be converted into either neutral or extreme positive.
        // Logic is the same as the whole-axis case.
        constexpr int32_t kAllowedValues[] = {kAnalogValueMin, kAnalogValueNeutral, kAnalogValueNeutral};
        int currentAllowedIndex = 0;
    ...
    // Verifies correct behavior when multiple digital axis mappers all contribute to the same virtual axis.
    ...
    // Verifies correct behavior when multiple digital axis mappers all contribute to the same virtual axis but the net contribution sums to the neutral position.
    ...
    // Verifies the nominal behavior in which a digital axis mapper is asked to contribute some arbitrary button press state to an axis.
    ...
    // Verifies correct behavior when multiple digital axis mappers all contribute to the same virtual axis but sourced by a button state.
    ...
    // Verifies the nominal behavior in which a digital axis mapper is asked to contribute a trigger value to an axis.
    // Sweeps the entire range of possible trigger values.
    // The only valid values that can be produced are extreme negative and extreme positive, and these values must exist in that order.
    TEST_CASE(DigitalAxisMapper_ContributeFromTriggerValue_Nominal_EntireAxis)
    {
        constexpr EAxis kTargetAxis = EAxis::RotX;

        // Same logic applies as in the analog value case, except that there is no neutral value because triggers do not have a neutral centered position.
    ...
    // Same as above, but for a half axis in the positive direction.
    // Here, the produced value is either neutral (trigger not pressed) or extreme positive (trigger is pressed).
    ...
    // Same as above, but for a half axis in the negative direction.
    // Here, the produced value is either neutral (trigger not pressed) or extreme negative (trigger is pressed).
    ...
    // Verifies correct behavior when multiple digital axis mappers all contribute to the same virtual axis but sourced by a trigger value.
