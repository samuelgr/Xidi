 ...
 *   Unit tests for controller element mappers that forward input received
 *   to multiple underlying element mappers.
 *****************************************************************************/
...
namespace XidiTest
{
    ...
    // Creates one CompoundMapper with an array of underlying element mappers present, some of which are null.
    ...
    // Creates and then clones one CompoundMapper with an array of underlying element mappers present, some of which are null.
    // Verifies correct reporting of the target elements.
    ...
    // Verifies correct routing of contributions from an analog source to all underlying element mappers.
    TEST_CASE(CompoundMapper_Route_Analog)
    {
        for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
        {
            ...
            static_assert(kExpectedContributionCount <= CompoundMapper::kMaxUnderlyingElementMappers, "Too many underlying element mappers are present.");

            int actualContributionCount = 0;

            CompoundMapper::TElementMappers mockElementMappers;
            for (int i = 0; i < kExpectedContributionCount; ++i)
                mockElementMappers[i] = std::make_unique<MockElementMapper>(MockElementMapper::EExpectedSource::Analog, (int16_t)analogValue, &actualContributionCount);
    ...
    // Verifies correct routing of contributions from a button source to all underlying element mappers.
    TEST_CASE(CompoundMapper_Route_Button)
    {
        ...
        for (bool buttonValue : kButtonValues)
        {
            ...
            CompoundMapper::TElementMappers mockElementMappers;
            for (int i = 0; i < kExpectedContributionCount; ++i)
                mockElementMappers[i] = std::make_unique<MockElementMapper>(MockElementMapper::EExpectedSource::Button, buttonValue, &actualContributionCount);
    ...
    // Verifies correct routing of contributions from a trigger source to all underlying element mappers.
    TEST_CASE(CompoundMapper_Route_Trigger)
    {
        for (int32_t triggerValue = kTriggerValueMin; triggerValue <= kTriggerValueMax; ++triggerValue)
        {
            ...
            CompoundMapper::TElementMappers mockElementMappers;
            for (int i = 0; i < kExpectedContributionCount; ++i)
                mockElementMappers[i] = std::make_unique<MockElementMapper>(MockElementMapper::EExpectedSource::Trigger, (uint8_t)triggerValue, &actualContributionCount);
    ...
    TEST_CASE(CompoundMapper_Route_Neutral)
    {
        ...
        CompoundMapper::TElementMappers mockElementMappers;
        for (int i = 0; i < kExpectedContributionCount; ++i)
            mockElementMappers[i] = std::make_unique<MockElementMapper>(MockElementMapper::EExpectedSource::Neutral, false, &actualContributionCount);
