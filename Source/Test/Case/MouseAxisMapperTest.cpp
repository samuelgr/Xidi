 ...
 *   Unit tests for controller element mappers that contribute movement of a
 *   virtual mouse along a virtual mouse axis.
 *****************************************************************************/
...
namespace XidiTest
{
    ...
    {
        constexpr EMouseAxis kTestMouseAxes[] = {EMouseAxis::X, EMouseAxis::Y, EMouseAxis::WheelHorizontal, EMouseAxis::WheelVertical};
    ...
    // Creates and then clones one mouse axis mapper for various possible mouse axes and verifies two things.
    // First, verifies that it does not map to any virtual controller element.
    // Second, verifies that it correctly identifies its target mouse axis.
    ...
    // Verifies the nominal behavior in which a mouse axis mapper is asked to contribute some arbitrary analog value to a mouse movement axis.
    ...
    TEST_CASE(MouseAxisMapper_ContributeFromAnalogValue_Nominal_EntireAxis)
    {
        ...
        for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
        {
            ...
            std::optional<int> maybeMouseMovementContribution = mockMouse.GetMovementContributionFromSource(kTargetAxis, kOpaqueSourceIdentifier);
    ...
    TEST_CASE(MouseAxisMapper_ContributeFromAnalogValue_Nominal_HalfAxisPositive)
    {
        ...
        for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
        {
            ...
            case kAnalogValueNeutral:
                TEST_ASSERT(((kMouseMovementUnitsNeutral + kMouseMovementUnitsMax) / 2) == mouseMovementContribution);
                break;
    ...
    TEST_CASE(MouseAxisMapper_ContributeFromAnalogValue_Nominal_HalfAxisNegative)
    {
        ...
        for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
        {
            ...
            case kAnalogValueNeutral:
                TEST_ASSERT(((kMouseMovementUnitsMin + kMouseMovementUnitsNeutral) / 2) == mouseMovementContribution);
                break;
    ...
    // Verifies the nominal behavior in which a mouse axis mapper is asked to contribute some arbitrary button press state to a mouse movement axis.
    ...
    // Verifies that a mouse axis mapper produces the same magnitude of motion in both directions when in half-axis mode.
    ...
    TEST_CASE(MouseAxisMapper_ContributeFromTriggerValue_Nominal_EntireAxis)
    {
        ...
        {
            SState unusedState;
            mapper.ContributeFromTriggerValue(unusedState, (uint8_t)triggerValue, kOpaqueSourceIdentifier);
    ...
    TEST_CASE(MouseAxisMapper_ContributeFromTriggerValue_Nominal_HalfAxisNegative)
    {
        ...
        for (int32_t triggerValue = kTriggerValueMin; triggerValue <= kTriggerValueMax; ++triggerValue)
        {
            ...
            // Triggers assigned to half-axes are special cases. All other test cases check for monotonic increase.
            int mouseMovementContribution = maybeMouseMovementContribution.value();
            TEST_ASSERT(mouseMovementContribution <= lastMouseMovementContribution);
            lastMouseMovementContribution = mouseMovementContribution;
