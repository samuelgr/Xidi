...
namespace XidiTest
{
    ...
    // Verifies the nominal behavior in which a button mapper is asked to contribute some arbitrary analog value to a button.
    // Expected behavior is the button is pressed at the extreme analog values and not pressed towards neutral, but the exact transition thresholds are not defined.
    ...
    TEST_CASE(ButtonMapper_ContributeFromAnalogValue_Nominal)
    {
        ...
        for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
        {
            ...
            possibleExpectedStates[0].button[(int)kTargetButton] = kExpectedButtonSequence[currentSequenceIndex];
            possibleExpectedStates[1].button[(int)kTargetButton] = kExpectedButtonSequence[currentSequenceIndex + 1];
    ...
    // Verifies correct behavior when multiple button mappers all contribute to the same virtual button with neutral analog values as input.
    // The aggregated contribution should always be that the button is not pressed, since no mapper sees any analog value away from neutral.
    TEST_CASE(ButtonMapper_ContributeFromAnalogValue_AllNeutral)
    {
        ...
        for (auto& mapper : mappers)
            mapper.ContributeFromAnalogValue(kControllerIdentifier, actualState, kAnalogValueNeutral);

        TEST_ASSERT(actualState == expectedState);
    }
    
    // Verifies correct behavior when multiple button mappers all contribute to the same virtual button with an extreme analog value as input.
    // The aggregated contribution should always be that the button is pressed.
    TEST_CASE(ButtonMapper_ContributeFromAnalogValue_ConstructiveInterference)
    {
        ...
        ZeroMemory(&actualState, sizeof(actualState));
        for (auto& mapper : mappers)
            mapper.ContributeFromAnalogValue(kControllerIdentifier, actualState, kAnalogValueMax);

        TEST_ASSERT(actualState == expectedState);
    }

    // Verifies correct behavior when multiple button mappers all contribute to the same virtual button but the net analog value sum equals the neutral position.
    // For button mappers this does not matter and the expected output is still that the button is pressed.
    TEST_CASE(ButtonMapper_ContributeFromAnalogValue_DestructiveInterference)
    {
        ...
        for (auto& mapper : mappersNegative)
            mapper.ContributeFromAnalogValue(kControllerIdentifier, actualState, kAnalogValueMin);

        TEST_ASSERT(actualState == expectedState);
    }

    // Verifies the nominal behavior in which a button mapper is asked to contribute some arbitrary button press state to a button.
    ...
    // Verifies correct behavior when multiple button mapper contributions occur to the same virtual button but mappers receive different input state.
    // As long as one button mapper receives an input of "pressed" then the virtual button should also be pressed.
    ...
    // Verifies the nominal behavior in which a button mapper is asked to contribute a trigger value to a button.
    // Expected behavior is the button is not pressed at the start and becomes pressed once the trigger value hits a threshold, but the exact transition point is not defined.
    ...
    // Verifies correct behavior when multiple button mappers all contribute to the same virtual button with minimum trigger values as input.
    // The aggregated contribution should always be that the button is not pressed, since no mapper sees any trigger value that could possibly have exceeded the threshold.
    TEST_CASE(ButtonMapper_ContributeFromTriggerValue_NonePressed)
    {
        ...
        ZeroMemory(&actualState, sizeof(actualState));
        for (auto& mapper : mappers)
            mapper.ContributeFromTriggerValue(kControllerIdentifier, actualState, kTriggerValueMin);

        TEST_ASSERT(actualState == expectedState);
    }

    // Verifies correct behavior when multiple button mappers all contribute to the same virtual button with maximum trigger values.
    // The aggregated contribution should always be that the button is pressed.
    TEST_CASE(ButtonMapper_ContributeFromTriggerValue_AllPressed)
    {
        ...
        ZeroMemory(&actualState, sizeof(actualState));
        for (auto& mapper : mappers)
            mapper.ContributeFromTriggerValue(kControllerIdentifier, actualState, kTriggerValueMax);

        TEST_ASSERT(actualState == expectedState);
    }

    // Verifies correct behavior when multiple button mappers all contribute to the same virtual button and only some are considered pressed based on the input trigger value.
    // For button mappers this does not matter and the expected output is still that the button is pressed.
