...
namespace Xidi
{
    ...
    inline constexpr GUID kVirtualControllerBaseGuid = {((0x0000 << 16) | (unsigned long)kVirtualControllerVendorId), 0x0000, 0x0000, {'X', 'I', 'D', 'I', 'V', 'C', 0x00, 0x00}};

    /// Force feedback driver GUID for Xidi virtual controllers.
    inline constexpr GUID kVirtualControllerForceFeedbackDriverGuid = {((0xffff << 16) | (unsigned long)kVirtualControllerVendorId), 0x0000, 0x0000, {'X', 'I', 'D', 'I', 'F', 'F', 'D', 0x00}};
    ...
    struct SHidUsageData
    {
        ...
        constexpr inline bool operator==(const SHidUsageData& other) const = default;
    };
    ...
    template <typename StringType> int FillHidCollectionName(StringType buf, size_t bufcount, uint16_t hidCollectionNumber);
    ...
    /// Obtains HID usage data for the specified controller element or, if the entire controller is requested, then obtains HID usage data for the virtual controller itself.
    ...
    constexpr inline uint16_t VirtualControllerProductId(Controller::TControllerIdentifier controllerId)
    {
