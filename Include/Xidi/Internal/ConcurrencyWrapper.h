...
namespace Xidi
{
    /// Wraps data in a way that is concurrency-safe following a single-producer multiple-consumer threading model.
    ...
    public:
        ...
        /// Updates the stored data in a concurrency-safe way and notifies all waiting threads of the change.
        /// Operations are conditional on the new data being different than the currently-stored data.
        /// @param [in] newData New data to be stored.
        /// @return `true` if the new data differ from the old and hence an update was performed, `false` otherwise.
        inline bool Update(const DataType& newData)
        {
            // This unguarded read is safe because by design only one thread, the one that produces updated data, ever invokes this method.
        ...
        /// @param [in,out] externalData On input, used to identify the last-known data for the calling thread. On output, filled in with the updated data.
        /// @param [in] stopToken Token that allows the wait to be interrupted.
        /// @return `true` if the wait succeeded and an update occurred, `false` if no updates were made due to invalid parameter or interrupted wait.
        inline bool WaitForUpdate(DataType& externalData, std::stop_token stopToken)
        {
            std::shared_lock lock(mutex);

            updateNotifier.wait(lock, stopToken, [this, &externalData]() -> bool
                {
